...
SELECT f.name, CONCAT(m.firstname, ' ', m.surname) AS MemberName, f.membercost * b.slots AS Cost
FROM Members AS m
INNER JOIN Bookings AS b
ON m.memid = b.memid
INNER JOIN Facilities AS f
ON b.facid = f.facid
WHERE b.memid <> 0 AND f.membercost * b.slots > 30 AND b.starttime LIKE '2012-09-14%' 

UNION

SELECT f.name, CONCAT(m.firstname, ' ', m.surname) AS MemberName, f.guestcost * b.slots
FROM Members AS m
INNER JOIN Bookings AS b
ON m.memid = b.memid
INNER JOIN Facilities AS f
ON b.facid = f.facid
WHERE b.memid = 0 AND f.guestcost * b.slots > 30 AND b.starttime LIKE '2012-09-14%' 
...
SELECT DISTINCT subquery.name, subquery.MemberName, subquery.Cost
FROM (
